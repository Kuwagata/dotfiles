...
__wezterm_set_user_var() {
  printf "\033]1337;SetUserVar=%s=%s\007" "$1" "$2"#`echo -n "$2" | base64`
}

__wezterm_set_user_var_old() {
  if [[ -z "${TMUX}" ]] ; then
...
#precmd_functions+=(__wezterm_semantic_precmd __wezterm_user_vars_precmd __wezterm_osc7)
#preexec_functions+=(__wezterm_semantic_preexec __wezterm_user_vars_preexec)
